...
(e.g. SMTP hostname and other basic info). The class also performs some analysis data points, like
the browser user-agents and IP addresses, to generate statistics for browser versions, operating
...
@click.option('--company', type=click.Choice(['Fraunhofer', 'HSMW', 'Axilaris']), help="Use this option to \
choose between report company template.", required=True)
@click.pass_context
def parse_options(self, id, format, combine, complete, config, google, verbose, company):
    ...
    gophish = goreport.Goreport(format, config, google, verbose, company)
